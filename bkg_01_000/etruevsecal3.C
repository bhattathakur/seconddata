void etruevsecal3()
{
  const char * filedata="DATA/E_estandE_true3.dat"; //File storing the estimated and true energy and their errors
  const char * rootfile="ROOTFILES/true_estimated_enregyplot3.root"; //root file to save the plot
  const char * filenamee="DATA/slope_intercept3.dat"; //file to store slope and intercept of plot
  const char * pdfetrue="PLOTS/EtrueECalc3.pdf"; //pdf file location
  c=new TCanvas();
  c->SetGrid();
  c->SetFillColor(42);
